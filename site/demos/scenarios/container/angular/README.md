...
Angular signal queries return `ElementRef<HTMLElement>` values. A computed signal resolves `nativeElement` for the selected query and passes it through the public `sourceRoot` and `scrollRoot` inputs.
